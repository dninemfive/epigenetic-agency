...
class Decider_CLI(Decider):
    def select_weapon(weapons: dict[str, Weapon], enemies: list[Enemy]) -> Weapon:
        ...
        weaponName: str = None
        while weaponName not in weapons.keys():
            weaponName = input()
    ...
    def target_enemies(weapon: Weapon, enemies: dict[str, Enemy]) -> list[Enemy]:
        ...
        availableEnemies: list[Enemy] = enemies.values()
        result: list[Enemy] = []
        ...
        while remaining_attacks > 0:
            ...
            enemyName: str = None
            while enemyName not in enemies.keys():
                pass
...
def battle(player: Player, enemies: dict[str, Enemy]) -> None:
    ...
    def do_turn():
        ...
        targets: list[Enemy] = player.decider.target_enemies(weapon, enemies.values())
